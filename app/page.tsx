...
const REVIEWS = [
  { name: 'Alewyn Bronn', job: 'Solar panel cleaning — Engen Petrol Station, Touws River (232 panels)' },
  { name: 'Rene Janse van Rensburg', job: 'Solar panel cleaning — Helderberg Retirement Village' },
  { name: 'Marlene Bronn', job: 'NGMS client' },
]
  ...
  return (
    <main>
      ...
      <section className="bg-jet text-white py-14 text-center px-4">
        ...
        <div className="max-w-5xl mx-auto grid sm:grid-cols-3 gap-6 text-left">
          {REVIEWS.map((review) => (
            <div key={review.name} className="bg-graphite border border-gray-700 rounded-xl p-6">
              <p className="text-orange text-lg mb-3">&#9733;&#9733;&#9733;&#9733;&#9733;</p>
              <p className="font-bold">{review.name}</p>
              <p className="text-gray-400 text-sm mt-1">{review.job}</p>
            </div>
          ))}
        </div>
      </section>
